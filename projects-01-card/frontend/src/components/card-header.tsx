...
export const CardHeader = ({ user }: { user?: IUserSummary | null }) => {
  ...
  return (
    <div className={"pt-6 pb-2 px-4 flex flex-col gap-2"}>
      <div
        className={cn(
          "flex justify-center items-center gap-1 text-black font-light",
        )}
      >
      ...
      <div className={"flex items-center justify-between  text-xs"}>
        ...
        <div className={"flex items-center gap-2"}>
          ...
          <div
            className={"shrink-0 text-lg font-medium text-primary-foreground"}
          >
            {config.name}
          </div>
        </div>

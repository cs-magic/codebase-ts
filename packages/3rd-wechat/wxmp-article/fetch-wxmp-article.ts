...
const fetchWxmpArticleViaFastAPI = async (
  url: string,
...
const fetchWxmpArticleViaNodeJS = async (
  url: string,
  options?: ICardGenOptions,
): Promise<IFetchWxmpArticleRes> => {
  // 1. fetch page
...
export const fetchWxmpArticle = async (
  ...
  options?.backendEngineType === "fastapi"
    ? fetchWxmpArticleViaFastAPI(url, options)
    : fetchWxmpArticleViaNodeJS(url, options)

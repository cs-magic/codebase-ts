...
export const requestPage = async (url, options) => {
    ...
    if (options?.approach?.type === "simulate") {
        ...
        pageText = (await api.get(url)).data;
    }
    ...
    const title = await withError("should title is valid")(z.string().min(1).parseAsync(parseMetaFromHtml(html, "og:title")));
    const coverUrl = await withError("should cover is valid")(z.string().min(1).parseAsync(parseMetaFromHtml(html, "og:image")));
    const description = await withError("should desc is valid")(z
        .string()

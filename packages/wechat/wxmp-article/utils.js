...
export const getWechatArticleUrlFromShortId = (shortId) => `https://mp.weixin.qq.com/s/${shortId}`;
export const parseWxmpArticleShortId = (url) => /mp.weixin.qq.com\/s\/(.*?)$/.exec(url)?.[1];
export const parseWxmpArticleLongId = (url) => /mp.weixin.qq.com.*?sn=(.*?)&/.exec(url)?.[1];
/**
 ...
 * @param url
 */
export const ensureWxmpArticleLongId = async (url) => {
    let longId = parseWxmpArticleLongId(url);
    // console.log({ url, longId })
    if (!longId) {
        const { data: pageText } = await api.get(url);
        ...
        // console.log({ newUrl })
        if (newUrl)
            longId = parseWxmpArticleLongId(newUrl);
        // console.log({ longId })
    }
    if (!longId)
        throw new Error(`failed to parse longId from url: ${url}`);
    return longId;
};

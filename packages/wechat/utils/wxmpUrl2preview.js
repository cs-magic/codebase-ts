...
export const wxmpUrl2preview = async (url, fetchOptions) => {
    if (/\/s\?/.test(url)) {
    ...
    const llmResponse = parseJsonSafe(result.llmResponse.response);
    const response = llmResponse?.response;
    if (!response)
        throw new Error("llm no response");
    const inner = {
        id: result.llmResponse.id,
        author: parseJsonSafe(result.article.author),
        cover: parseJsonSafe(result.article.cover),

...
import { GenWxmpArticleCardFetchOptions } from "@cs-magic/swot-core/src/schema"
...
// import { GenWxmpArticleCardFetchOptions } from "../../src/schema/card"
...
export const fetchWxmpArticle = async (
  ...
  if (!article) {
    article = await prisma.card.create({
      data: await request(url, options?.detail?.request),
    })
  }

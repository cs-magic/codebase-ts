...
test('resolvePuppet() for supported/unsupported name', async (t) => {
    await t.rejects(() => resolvePuppet({ puppet: 'fadfdsafa' }), 'reject when options.puppet is unknown');
    await t.resolves(() => resolvePuppet({ puppet: 'yocto-queue' }), 'should resolve a valid npm package as puppet name');
});
test('resolvePuppetName() for ESM', async (t) => {
    const PuppetConstructor = await resolvePuppetName('yocto-queue');
...
test('resolvePuppetName() for CJS', async (t) => {
    const PuppetConstructor = await resolvePuppetName('nop');
    t.equal(typeof PuppetConstructor, 'function', 'should get the puppet class function');

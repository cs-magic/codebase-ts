...
export const passwordGenerator = (platformName: string, pattern: PasswordPattern) => {
  ...
  switch (pattern) {
    case PasswordPattern.Ascii:
      codeSection = calcCode(platformNameSection, 97).toString().padStart(2, "0")
      break
    case PasswordPattern.ByBit:
      codeSection = [2, 3, 5, 7].map((c) => platformNameSection.length % c).join("")
  }

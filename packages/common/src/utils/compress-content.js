export function compressContent(content, targetLen = 6000) {
    const horizontalCompress = () => {
        const compressLine = (line, ratio = 0.9) => {
            if (line.length < 20) {
                return line;
            }
            const n = line.length;
            return (line.substring(0, Math.floor((n * ratio) / 2)) +
                "……" +
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            console.debug(`[${i}] ${lines.length} lines, ${chars1} --> ${chars2}, ratio: ${((chars2 / chars1) * 100).toFixed(2)}%`);
        }
    };

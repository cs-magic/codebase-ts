...
export type ITransEvent<T extends TransEventType = any> = T extends "init" ? GenericTransEvent<"init", object> : T extends "data" ? GenericTransEvent<"data", {
    token: string;
    convId?: string;
}> : T extends "error" ? GenericTransEvent<"error", {
    message: string;
}> : T extends "close" ? GenericTransEvent<"close", {
    reason: ResponseFinalStatus;
}> : T extends "onClientConnected" ? GenericTransEvent<"onClientConnected", {
    id: string;
}> : T extends "onClientDisconnected" ? GenericTransEvent<"onClientDisconnected", {
    id: string;
}> : never;

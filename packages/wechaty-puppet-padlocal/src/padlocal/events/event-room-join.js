...
import { createSysmsgTemplateRunner, } from "../messages/sysmsg/message-sysmsgtemplate.js";
...
export default async (puppet, message) => {
     ...
     * /^" ?(.+)"通过扫描你分享的二维码加入群聊/,
     */
    const youInviteOther = createSysmsgTemplateRunner(sysmsgTemplatePayload, [...YOU_INVITE_OTHER_REGEX_LIST, ...OTHER_JOIN_VIA_YOUR_QRCODE_REGEX_LIST], async (templateLinkList) => {
     ...
     * /^"([^"]+?)"邀请你加入了群聊/,
     */
    const otherInviteYou = createSysmsgTemplateRunner(sysmsgTemplatePayload, OTHER_INVITE_YOU_REGEX_LIST, async (templateLinkList) => {
     ...
     * /^"(.+)"邀请"(.+)"加入了群聊/,
     */
    const otherInviteOther = createSysmsgTemplateRunner(sysmsgTemplatePayload, [...OTHER_INVITE_YOU_AND_OTHER_REGEX_LIST, ...OTHER_INVITE_OTHER_REGEX_LIST], async (templateLinkList, matchedRegexIndex) => {
     ...
     * /^" (.+)"通过扫描"(.+)"分享的二维码加入群聊/,
     */
    const otherJoinViaQrCode = createSysmsgTemplateRunner(sysmsgTemplatePayload, OTHER_JOIN_VIA_OTHER_QRCODE_REGEX_LIST, async (templateLinkList) => {

...
import type { ImageType } from "src/schemas/image"
...
import type { PostPayload } from "src/schemas/post"
import { type SayablePayload, sayableTypes } from "src/schemas/sayable"
...
const messageMixin = <MinxinBase extends typeof PuppetSkeleton & CacheMixin>(baseMixin: MinxinBase) => {
  abstract class MessageMixin extends baseMixin {
    ...
    async messagePayload(messageId: string): Promise<MessagePayload> {
    ...
    async messageSearch(query?: MessageQueryFilter): Promise<string[] /* Message Id List */> {
      ...
      const messagePayloadList: MessagePayload[] = await Promise.all(
        allMessageIdList.map((id) => this.messagePayload(id)),
      )
    ...
    messageQueryFilterFactory(query: MessageQueryFilter): MessagePayloadFilterFunction {
      ...
      const allFilterFunction: MessagePayloadFilterFunction = (payload) =>
        filterFunctionList.every((func) => func(payload))

      return allFilterFunction
    }

    async messagePayloadDirty(id: string): Promise<void> {

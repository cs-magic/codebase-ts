...
import type { ContactPayload, ContactPayloadFilterFunction, ContactQueryFilter } from "src/schemas/contact"
  ...
  abstract class ContactMixin extends mixinBase {
    ...
    async contactSearch(query?: string | ContactQueryFilter, searchIdList?: string[]): Promise<string[]> {
      log.verbose(
        ...
        searchIdList ? `idList.length = ${searchIdList.length}` : "",
      )
    ...
    async contactPayload(contactId: string): Promise<ContactPayload> {
    ...
    async contactPayloadDirty(id: string): Promise<void> {

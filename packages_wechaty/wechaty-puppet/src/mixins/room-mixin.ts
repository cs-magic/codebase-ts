...
import type { RoomPayload, RoomPayloadFilterFunction, RoomQueryFilter } from "src/schemas/room"

import type { ContactMixin } from "src/mixins/contact-mixin"
import type { RoomMemberMixin } from "src/mixins/room-member-mixin"
...
const roomMixin = <MixinBase extends typeof PuppetSkeleton & ContactMixin & RoomMemberMixin>(mixinBase: MixinBase) => {
  abstract class RoomMixin extends mixinBase {
    ...
    async roomPayload(roomId: string): Promise<RoomPayload> {
    ...
    async roomPayloadDirty(id: string): Promise<void> {

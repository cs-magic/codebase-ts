...
import type { Sayable } from "src/sayable/mod"
import { payloadToSayableWechaty, sayableToPayload } from "src/sayable/mod"
import { validationMixin, wechatifyMixinBase } from "src/user-mixins/mod"

import { ContactImpl } from "src/user-modules/contact"
import type { ContactInterface } from "src/user-modules/contact"
...
class PostBuilder {
   ...
   */
  static new(Impl: typeof PostMixin) {
    return new this(Impl)
  }
  protected constructor(protected Impl: typeof PostMixin) {
    this.payload = {
  ...
  async build(): Promise<PostInterface> {
    const sayablePayloadListNested = await Promise.all(this.sayableList.map(sayableToPayload))
...
class PostMixin extends wechatifyMixinBase() {
  ...
  static async find(filter: PUPPET.filters.Post): Promise<undefined | PostInterface> {
    log.verbose("Post", "find(%s)", JSON.stringify(filter))
  ...
  ): Promise<[postList: PostInterface[], nextPageToken?: string]> {
    log.verbose("Post", "findAll(%s%s)", JSON.stringify(filter), pagination ? ", " + JSON.stringify(pagination) : "")

    const { nextPageToken, response: postIdList } = await this.wechaty.puppet.postSearch(filter, pagination)

    const idToPost = async (id: string) => this.wechaty.Post.find({ id }).catch((e) => this.wechaty.emitError(e))
  ...
  constructor(idOrPayload: string | PUPPET.payloads.Post) {
    ...
    log.verbose(
      "Post",
      "constructor(%s)",
      typeof idOrPayload === "string" ? idOrPayload : JSON.stringify(idOrPayload.id),
    )
  ...
  async *likes(filter: PUPPET.filters.Post = {}): AsyncIterableIterator<Tap> {
  ...
  async *taps(filter: PUPPET.filters.Tap = {}): AsyncIterableIterator<Tap> {
  ...
  async reply(
    sayable: Exclude<Sayable, PostInterface> | Exclude<Sayable, PostInterface>[],
  ): Promise<void | PostInterface> {
    log.verbose("Post", "reply(%s)", sayable)

    if (!this.id) {
      console.error(
        "You can only call `reply()` on received posts, but it seems that you are trying to call reply on a post created from local.",
      )
      throw new Error("no post id found")
    }
    ...
    const post = await builder.reply(this).build()
  ...
  async tap(type: PUPPET.types.Tap, status?: boolean): Promise<void | boolean> {
    log.verbose("Post", "tap(%s%s)", PUPPET.types.Tap[type], typeof status === "undefined" ? "" : ", " + status)
  ...
  async tapFind(
    ...
    pagination?: PUPPET.filters.PaginationRequest,
  ): Promise<[tapList: Tap[], nextPageToken?: string]> {
...
type PostConstructor = Constructor<PostInterface, typeof PostImpl>

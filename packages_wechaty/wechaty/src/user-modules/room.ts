...
import { FOUR_PER_EM_SPACE, log } from "src/config"
import { stringifyFilter } from "src/helper-functions/stringify-filter"
import { guardQrCodeValue } from "src/pure-functions/guard-qr-code-value"
import { isTemplateStringArray } from "src/pure-functions/is-template-string-array"
import { wechatyCaptureException } from "src/raven"
import { deliverSayableConversationPuppet } from "src/sayable/mod"
import type { Sayable, SayableSayer } from "src/sayable/mod"
import { RoomEventEmitter } from "src/schemas/mod"
import { poolifyMixin, validationMixin, wechatifyMixin } from "src/user-mixins/mod"

import { ContactImpl, ContactInterface } from "src/user-modules/contact"
import type { MessageInterface } from "src/user-modules/message"

const MixinBase = wechatifyMixin(poolifyMixin(RoomEventEmitter)<RoomImplInterface>())
...
class RoomMixin extends MixinBase implements SayableSayer {
  ...
  async say(sayable: Sayable | TemplateStringsArray, ...varList: unknown[]): Promise<void | MessageInterface> {
    ...
    if (isTemplateStringArray(sayable)) {
      msgId = await this.sayTemplateStringsArray(sayable as TemplateStringsArray, ...varList)
      ...
      if (varList.length > 0) {
        ...
        const mentionAlias = await Promise.all(
          mentionList.map(async (contact) => "@" + ((await this.alias(contact)) || contact.name())),
        )
  ...
  private async sayTemplateStringsArray(textList: TemplateStringsArray, ...varList: unknown[]) {
  ...
  async topic(newTopic?: string): Promise<void | string> {
    ...
    if (typeof newTopic === "undefined") {
      if (this.payload && this.payload.topic) {
        ...
        const memberList = (await Promise.all(memberListFuture)).filter(Boolean) as ContactInterface[]
    ...
    const future = this.wechaty.puppet.roomTopic(this.id, newTopic).catch((e) => {
  ...
  async announce(text?: string): Promise<void | string> {
    log.verbose("Room", "announce(%s)", typeof text === "undefined" ? "" : `"${text || ""}"`)
  ...
  async has(contact: ContactInterface): Promise<boolean> {
    ...
    return memberIdList.filter((id) => id === contact.id).length > 0
  }
   ...
   */
  async memberAll(query?: string | PUPPET.filters.RoomMember): Promise<ContactInterface[]> {
   ...
   * }
   */
  async member(queryArg: string | PUPPET.filters.RoomMember): Promise<undefined | ContactInterface> {
    ...
    if (memberList.length > 1) {
      log.warn(
        "Room",
        "member(%s) get %d contacts, use the first one by default",
...
type RoomProtectedProperty = "ready"

type RoomInterface = Omit<RoomImplInterface, RoomProtectedProperty>

type RoomConstructor = Constructor<RoomImplInterface, Omit<typeof RoomImpl, "load">>

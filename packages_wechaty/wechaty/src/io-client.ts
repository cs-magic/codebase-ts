...
import { PuppetServer, PuppetServerOptions } from "packages_wechaty/wechaty-puppet-service/src/mod"
  ...
  private async hookWechaty(wechaty: WechatyInterface): Promise<void> {
    ...
    wechaty
      ...
      .on("scan", (url, code) => {
        log.info(
          "IoClient",
          [`[${code}] ${url}`, `Online QR Code Image: https://wechaty.js.org/qrcode/${encodeURIComponent(url)}`].join(
            "\n",
          ),
        )
      })
  ...
  public async quit(): Promise<void> {
    ...
    try {
      ...
      if (this.io) {
        ...
        log.warn("IoClient", "quit() no this.io")
      }
    } catch (e) {

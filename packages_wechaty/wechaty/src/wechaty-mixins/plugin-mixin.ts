...
import type { WechatyPlugin, WechatyPluginUninstaller } from "src/plugin"
import { isWechatyPluginUninstaller } from "src/plugin"
import type { WechatySkeleton } from "src/wechaty/mod"

import type { GErrorMixin } from "src/wechaty-mixins/gerror-mixin"
import type { MiscMixin } from "src/wechaty-mixins/misc-mixin"
    ...
    use(...plugins: (WechatyPlugin | WechatyPlugin[])[]): WechatyPluginUninstaller {
      ...
      for (const plugin of pluginList) {
        ...
        if (isWechatyPluginUninstaller(uninstaller)) {
          log.verbose(
            "WechatyPluginMixin",
            "use() saving uninstaller for Plugin %s on Wechaty %s ...",
            plugin.name,
            this.name(),
          )
       ...
       */
      return () =>
        uninstallerList.forEach((uninstaller) => {
          log.verbose(
            "WechatyPluginMixin",
            "use() uninstalling Plugin %s on Wechaty %s ...",
            uninstaller.name,
            this.name(),
          )
          uninstaller()
          log.verbose(
            "WechatyPluginMixin",
            "use() uninstalling Plugin %s on Wechaty %s ... done",
            uninstaller.name,
            this.name(),
          )
        })

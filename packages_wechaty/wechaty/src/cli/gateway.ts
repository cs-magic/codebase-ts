...
import { IoClient, IoClientOptions } from "src/io-client"
import { OFFICIAL_PUPPET_DEFAULT, OFFICIAL_PUPPET_DEPENDENCIES, isPuppetModuleName } from "src/puppet-config"
import { WechatyBuilder } from "src/wechaty-builder"

async function onError(this: IoClient, e: Error) {
...
const gateway = command({
  name: "gateway",
  description:
    "Publish your DIY token as a Wechaty Puppet Service. (you can generate a token by running `wechaty token generate`)",
  args: {
    token: option({
      description:
        "Specify a TOKEN for your DIY Wechaty Puppet Service (WPS). It is the standard token service for TypeScript and Polyglot Wechaty (like Python, Rust, Go, etc.). If it not specified, it will be generated automatically.",
      long: "token",
      short: "t",
    ...
    port: option({
      description:
        "Specify a Port for your DIY Wechaty Puppet Service (WPS). If it not specified, it will select a free port automatically. This port MUST be public accessiable via the internet.",
      long: "port",
      short: "p",
    ...
    puppet: option({
      description:
        "Specify a Wechaty Puppet Provider (WPP). If it not specified, it will use the environment variable `WECHATY_PUPPET`.",
      long: "puppet",
      short: "P",
      type: optional(string),
    }),
    puppetToken: option({
      description:
        "Specify the WEchaty Puppet Provider TOKEN. If it not specified, it will use the environment variable `WECHATY_PUPPET_XXX_TOKEN` (`XXX` is your WPP name).",
      long: "puppet-token",
      short: "T",
      type: optional(string),
    }),
  ...
  handler: async (args) => {
    ...
    if (!puppet) {
      log.info(
        "Gateway",
        "no `--puppet` specified, reading from environment variable `WECHATY_PUPPET` for Wechaty Puppet Provider ...",
      )
      puppet = process.env["WECHATY_PUPPET"]
      if (!puppet) {
        log.info(
          "Gateway",
          `environment variable "WECHATY_PUPPET" is not available. Using system default puppet "${OFFICIAL_PUPPET_DEFAULT}"`,
        )
    ...
    if (!puppetToken) {
      log.info(
        "Gateway",
        "no `--puppet-token` specified, reading from environment variable `WECHATY_PUPPET_TOKEN` for your Wechaty Puppet Provider ...",
      )

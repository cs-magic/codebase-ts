...
function getRetryPolicy(): RetryPolicy {
  ...
  policy.onRetry((reason) =>
    log.silly("wechaty", 'retry-policy getRetryPolicy policy.onRetry() reason: "%s"', JSON.stringify(reason)),
  )
  policy.onSuccess(({ duration }) =>
    log.silly("wechaty", "retry-policy getRetryPolicy policy.onSuccess(): retry call ran in %s ms", duration),
  )
  return policy
}

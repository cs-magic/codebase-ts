...
import jsonRpcPeer from "src/io-peer/json-rpc-peer"
...
const isJsonRpcRequest = (payload: jsonRpcPeer.JsonRpcPayload): payload is jsonRpcPeer.JsonRpcPayloadRequest =>
  "method" in payload
const isJsonRpcNotification = (
  payload: jsonRpcPeer.JsonRpcPayload,
): payload is jsonRpcPeer.JsonRpcPayloadNotification => isJsonRpcRequest(payload) && !("id" in payload)
const isJsonRpcResponse = (payload: jsonRpcPeer.JsonRpcPayload): payload is jsonRpcPeer.JsonRpcPayloadResponse =>
  "result" in payload
const isJsonRpcError = (payload: jsonRpcPeer.JsonRpcPayload): payload is jsonRpcPeer.JsonRpcPayloadError =>
  "error" in payload

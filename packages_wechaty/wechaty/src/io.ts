...
import { getPeer, isJsonRpcRequest } from "src/io-peer/io-peer"
...
export class Io {
  ...
  constructor(protected options: IoOptions) {
  ...
  private async wsOnMessageAsync(data: WebSocket.Data): Promise<void> {
    ...
    switch (ioEvent.name) {
      ...
      case "reset":
        ...
        this.options.wechaty.emitError(new Error("reset by server: " + JSON.stringify(ioEvent.payload)))
  ...
  private wsOnClose(ws: WebSocket, code: number, message: string): void {

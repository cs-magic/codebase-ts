...
import { poolifyMixin } from "src/user-mixins/poolify"
import { wechatifyMixin, wechatifyUserModule } from "src/user-mixins/wechatify"
...
test("poolifyMixin()", async (t) => {
  ...
  type PoolGetReturnType = Exclude<ReturnType<typeof UserClassTest.pool.get>, undefined>
  const poolGetType: AssertEqual<PoolGetReturnType, UserClassTest> = true
  t.ok(poolGetType, "should get Life type for pool.get()")

  type LoadReturnType = ReturnType<typeof UserClassTest.load>
  const loadType: AssertEqual<LoadReturnType, UserClassTest> = true
...
test("poolifyMixin() pool", async (t) => {
  ...
  class UserClassTest extends wechatifyMixin(poolifyMixin(Life)<UserClassTest>()) {
    id = 0
    constructor() {
      super()
    }
...
test("static findAll()", async (t) => {
  class UserClassTest extends wechatifyMixin(poolifyMixin(Object)<UserClassTest>()) {
    static async findAll<T extends typeof UserClassTest>(this: T, id: string) {
      return this.load(id)
    }
  ...
  type findAllReturnType = ReturnType<typeof UserClassTest.findAll>
  const typeTest: AssertEqual<findAllReturnType, Promise<UserClassTest>> = true

...
} from "packages_wechaty/wechaty_grpc/src/mod";
...
} from "src/deprecated/mod";

import { timestampFromMilliseconds } from "src/pure-functions/timestamp";
import { normalizeFileBoxUuid } from "src/file-box-helper/mod";
import { log } from "src/config";
import { grpcError } from "src/server/grpc-error";
import { EventStreamManager } from "src/server/event-stream-manager";
...
function puppetImplementation(
  ...
  puppet.on("login", (_) => {
    ...
    readyTimeout = setTimeout(() => {
      ...
      readyPayload &&
        eventStreamManager.grpcEmit(
          grpcPuppet.EventType.EVENT_TYPE_READY,
          readyPayload,
        );
    }, 5 * 1000);
  });
  ...
  const puppetServerImpl: grpcPuppet.IPuppetServer = {
    ...
    contactCorporationRemark: async (call, callback) => {
      ...
      try {
        ...
        return callback(
          null,
          new grpcPuppet.ContactCorporationRemarkResponse(),
        );
      } catch (e) {
    ...
    event: (streamingCall) => {
      ...
      if (eventStreamManager.busy()) {
        log.error(
          "PuppetServiceImpl",
          "event() there is another event() call not end when receiving a new one.",
        );
      ...
      if (scanPayload) {
        eventStreamManager.grpcEmit(
          grpcPuppet.EventType.EVENT_TYPE_SCAN,
          scanPayload,
        );
      }
    },
    ...
    friendshipAdd: async (call, callback) => {
      ...
      try {
...

        {
          ...
          const sourceContactId = call.request
            .getSourceContactIdStringValueDeprecated()
            ?.getValue();
          const sourceRoomId = call.request
            .getSourceRoomIdStringValueDeprecated()
            ?.getValue();
          if (sourceContactId) {
    ...
    friendshipPayload: async (call, callback) => {
      ...
      try {
        ...
        response.setId(payload.id);
        response.setScene(
          payloadReceive.scene || PUPPET.types.FriendshipScene.Unknown,
        );
    ...
    friendshipSearchWeixin: async (call, callback) => {
      log.warn(
        "PuppetServiceImpl",
        "friendshipSearchWeixin() is deprecated, use friendshipSearchHandle() instead. %s",
        new Error().stack,
      );
    ...
    messageFileStream: async (call) => {
      ...
      try {
        ...
        const fileBox = await puppet.messageFile(id);
        const response = await packFileBoxToPb(
          grpcPuppet.MessageFileStreamResponse,
        )(fileBox);
        ...
        log.error(
          "PuppetServiceImpl",
          "grpcError() messageFileStream() rejection: %s",
    ...
    messageImageStream: async (call) => {
      ...
      try {
        ...
        const response = await packFileBoxToPb(
          grpcPuppet.MessageImageStreamResponse,
        )(fileBox);
        ...
        log.error(
          "PuppetServiceImpl",
          "grpcError() messageImageStream() rejection: %s",
          (e as Error).message,
        );
        call.destroy(e as Error);
      }
    ...
    messagePayload: async (call, callback) => {
      ...
      try {
        ...
        const mentionIdList =
          "mentionIdList" in payload ? payload.mentionIdList || [] : [];
        ...
        response.setType(
          payload.type as grpcPuppet.MessageTypeMap[keyof grpcPuppet.MessageTypeMap],
        );
    ...
    messageSendContact: async (call, callback) => {
      ...
      try {
        ...
        const messageId = await puppet.messageSendContact(
          conversationId,
          contactId,
        );
    ...
    messageSendLocation: async (call, callback) => {
      ...
      try {
        ...
        const messageId = await puppet.messageSendLocation(
          conversationId,
          payload,
        );
    ...
    messageSendMiniProgram: async (call, callback) => {
      ...
      try {
        ...
        const messageId = await puppet.messageSendMiniProgram(
          conversationId,
          payload,
        );
    ...
    messageSendText: async (call, callback) => {
      ...
      try {
        ...
        const messageId = await puppet.messageSendText(
          conversationId,
          text,
          mentionIdList,
        );
      ...
      try {
         ...
         */
        {
          ...
          if (jsonText) {
            ...
            return callback(
              null,
              new grpcPuppet.RoomInvitationPayloadResponse(),
            );
          }

          {
            ...
            const payloadWrapper =
              call.request.getPayloadStringValueDeprecated();
    ...
    stopGeneration: async (call, callback) => {
      ...
      try {
        if (eventStreamManager.busy()) {
          ...
          log.error(
            "PuppetServiceImpl",
            "stop() eventStreamManager is not busy?",
          );
        }
      ...
      try {
...

        {
          ...
          const contactIdWrapper =
            call.request.getContactIdStringValueDeprecated();

...
export default function SpiderPage() {
  ...
  useEffect(() => {
    ...
    inputCookie.split(";").forEach((line) => {
      const [key, value] = line.split("=", 2)
      // console.log({ line, key, value })
      if (key && value) parsedCookie.push({ key, value, status: "to-validate" })
    })
  ...
  const validateCookie = async (cookies: ICookie[]) => {
    for (const cookie of cookies) {
      ...
      const data = await serverFetch(url, cookie2headers(cookiesToSend))
  ...
  return (
    <FlexContainer
      ...
      className={"h-full overflow-auto justify-start"}
    >

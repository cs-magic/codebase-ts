...
class PuppetTest extends PUPPET.Puppet {
   ...
   *
   */
  override async tagContactAdd(id: string, contactId: string): Promise<void> {
    return void { contactId, id }
  }
  ...
  override async tagContactDelete(id: string): Promise<void> {
    return void { id }
  }
  ...
  override async contactAvatar(
    ...
    return { contactId, file } as any
  }

  override async contactPhone(
    contactId: string,
    phoneList: string[],
  ): Promise<void> {
    return { contactId, phoneList } as any
  }
  ...
  override async contactCorporationRemark(
    contactId: string,
    corporationRemark: string | null,
  ): Promise<void> {
    return { contactId, corporationRemark } as any
  }

  override async contactDescription(
    contactId: string,
    description: string | null,
  ): Promise<void> {
    return { contactId, description } as any
  }
  ...
  override async friendshipSearchPhone(phone: string): Promise<string | null> {
    return phone
  }
  ...
  override async messageImage(
    ...
    return { imageType, messageId } as any
  }
  ...
  override async messageForward(
    ...
    return { messageId, to } as any
  }
  override async messageSendContact(
    receiver: string,
    contactId: string,
  ): Promise<void | string> {
    return { contactId, receiver } as any
  }
  override async messageSendFile(
    to: string,
    file: FileBoxInterface,
  ): Promise<void | string> {
    return { file, to } as any
  }
  override async messageSendText(
    to: string,
    text: string,
  ): Promise<void | string> {
    return { text, to } as any
  }
  ...
  override async messageSendMiniProgram(
    ...
    return { miniProgramPayload, to } as any
  }
  override async messageSendLocation(
    conversationId: string,
    locationPayload: PUPPET.payloads.Location,
  ): Promise<void | string> {
    return { conversationId, locationPayload } as any
  }
  ...
  override async conversationReadMark(
    ...
    void conversationId, void hasRead
  }
  ...
  override async roomInvitationAccept(roomInvitationId: string): Promise<void> {
    void roomInvitationId
  }

  override async roomInvitationRawPayload(
    roomInvitationId: string,
  ): Promise<any> {
    return { roomInvitationId } as any
  }
  ...
  override async roomRawPayload(id: string): Promise<any> {
    return { id } as any
  }
   ...
   */
  public override roomQueryFilterFactory(query: PUPPET.filters.Room): any {
...
async function main() {
  ...
  if (PUPPET.VERSION === "0.0.0" || puppetInstance.name() === "NONAME") {
    throw new Error(
      "the `src/package-json.ts` has not been generated correctly.",
    )
  }
  ...
  assert.strictEqual(
    puppetInstance.name(),
    "wechaty-puppet",
    "should get base class name",
  )
  assert.notStrictEqual(
    puppetInstance.version(),
    "0.0.0",
    "version should not be 0.0.0 when publishing",
  )
